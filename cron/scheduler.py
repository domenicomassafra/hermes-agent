...
def _automation_recap_renderer() -> Path:
    """Locate the one owner-approved renderer without copying its template."""
    configured = os.environ.get("HERMES_TELEGRAM_RECAP_RENDERER", "").strip()
    return Path(configured).expanduser() if configured else Path.home() / "ControlCenter" / "tools" / "telegram_recap.py"


def _automation_delivery_refusal(job: dict, reason: str) -> str:
    """Safe fallback when a scheduled sender is not contract-compliant.

    The detailed stdout remains in the local cron receipt.  It must never be
    forwarded as a technical Telegram message.
    """
...
def _render_automation_delivery(job: dict, output: str) -> str:
    ...
    try:
        completed = subprocess.run(
            ...
            text=True, timeout=15, check=False, creationflags=windows_hide_flags(),
        )
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    for target in targets:
        ...
        if runtime_adapter is not None and loop is not None and getattr(loop, "is_running", lambda: False)():
            ...
            # A positive Telegram chat id plus a numeric topic is ambiguous:
            # probe the live adapter once and use the result for both text and
            # media.  Unknown/error is fail-safe message_thread_id.
            route_thread_id = str(thread_id) if thread_id is not None else None
            route_metadata = {"job_id": job["id"]}
            if route_thread_id is not None:
                # Mark the ordinary thread route explicitly so DeliveryRouter
                # does not repeat this probe before the text send.
                route_metadata["thread_id"] = route_thread_id
            media_metadata = {"thread_id": thread_id} if thread_id else None
            if (
                platform == Platform.TELEGRAM
                and thread_id is not None
                and str(chat_id).lstrip("+").isdigit()
                and str(thread_id).isdigit()
            ):
                try:
                    from agent.async_utils import safe_schedule_threadsafe
                    probe = safe_schedule_threadsafe(
                        runtime_adapter.get_chat_info(str(chat_id)), loop
                    )
                    chat_info = probe.result(timeout=5) if probe is not None else None
                except Exception:
                    chat_info = None
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    # Cron may run outside the gateway process (manual fire, scheduler worker).
    # Refresh the canonical owner dotenv before resolving providers or delivery
    # homes; an absent/ambiguous target remains undelivered downstream.
    _env_loader.reset_secret_source_cache()
    _env_loader.load_hermes_dotenv(hermes_home=_hermes_home)
    ...
    if job.get("no_agent"):
        ...
        if not ok:
            ...
            return False, doc, (_render_automation_delivery(job, output) if _requires_automation_recap(job) else alert), output
        ...
        final_response = _render_automation_delivery(job, output) if _requires_automation_recap(job) else output
